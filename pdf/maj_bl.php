...
$sql_mt_total=$pdo->query("SELECT X.num_bl, SUM(total) as mt_total from (select D.num_bl, ROUND(count(S.reference)*D.prix_vente, 2) as total from vente_details D   
                            ...
                            inner join articles A on A.id = V.id_articles GROUP BY V.num_bl) X where X.num_bl='".$num_bl."'");

$mt_total = $sql_mt_total->fetch(PDO::FETCH_ASSOC);
$mt_total_ttc = $mt_total['mt_total'];
$mt_total_ht = $mt_total['mt_total']/1.2;
$req_date = $pdo-> query("SELECT * FROM `bl` WHERE num_bl = '$num_bl'");
...
class myPDF extends FPDF{
    ...
    function viewTable($bl_info, $especes, $mt_total_ht, $mt_total_ttc, $num_cheque, $mt_cheque){
...
$pdf->Ln(10);
$pdf->headerTable();
$pdf->viewTable($bl_info, $especes, $mt_total_ht, $mt_total_ttc, $num_cheque, $mt_cheque);

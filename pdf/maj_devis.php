...
$req_mt_ttc = $pdo-> query("SELECT SUM(quantite*prix_ttc) as total FROM `devis_details` WHERE num_devis = '$num_devis'");
...
$req_date = $pdo-> query("SELECT date_devis FROM `devis` WHERE num_devis = '$num_devis'");
$date = $req_date->fetch(PDO::FETCH_ASSOC);
$date = $date['date_devis'];
$devis_info = $pdo-> query("SELECT reference, type, quantite, prix_ttc/1.2 as prix FROM devis_details WHERE num_devis = '$num_devis'");
